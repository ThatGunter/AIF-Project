...
from keybinds import KeybindsUI
...
class Level:
    def __init__(self, tmx_map, level_frames, data, switch_stage): 
        ...
        self.keybinds = KeybindsUI()

        self.level_start_timer = Timer(5000)
        self.level_start_timer.activate()
    ...
    def setup(self, tmx_map, level_frames):
        ...
        for obj in tmx_map.get_layer_by_name('BG details'):
            if obj.name == 'static':
                ...
                AnimatedSprite((obj.x, obj.y), level_frames[obj.name], self.all_sprites, z = Z_LAYERS['bg details'], animation_speed = ANIMATION_SPEED)
                if obj.name == 'candle':
                    AnimatedSprite((obj.x, obj.y) + vector(-20, -20), level_frames['candle_light'], self.all_sprites, Z_LAYERS['bg details'])
    ...
    def attack_collision(self):
        for target in self.pearl_sprites.sprites() + self.tooth_sprites.sprites():
            facing_target = self.player.rect.centerx < target.rect.centerx and self.player.facing_right or \
                            self.player.rect.x > target.rect.centerx and not self.player.facing_right
    ...
    def draw_level_start_message(self):
        if self.show_level_message and self.level_start_timer.active:
            font = pygame.font.Font(None, 48)
            message = "FIND THE FLAG!"
            
            text_surf = font.render(message, True, 'white')
            text_rect = text_surf.get_frect(center=(WINDOW_WIDTH // 2, 50))
    ...
    def run(self, dt): #Printing displays
        self.display_surface.fill('gray')

        self.level_start_timer.update()
        if not self.level_start_timer.active:
        ...
        self.draw_level_start_message()
        if self.showkeybinds:
            self.keybinds.draw()