...
class Tooth(pygame.sprite.Sprite):
    ...
    def update(self, dt):
        ...
        floor_rect_right = pygame.FRect(self.rect.bottomright + vector(-2, 0), (4, 8))
        floor_rect_left = pygame.FRect(self.rect.bottomleft + vector(-2, 0), (4, 8))
        wall_rect = pygame.FRect(self.rect.midright if self.direction > 0 else self.rect.midleft + vector(-2, 0),(2, self.rect.height // 2))
        

        has_floor_right = floor_rect_right.collidelist(self.collision_rects) >= 0
        has_floor_left = floor_rect_left.collidelist(self.collision_rects) >= 0
        hit_wall = wall_rect.collidelist(self.collision_rects) >= 0
        should_reverse = False

        if self.direction > 0 and not has_floor_right:
            should_reverse = True
        elif self.direction < 0 and not has_floor_left:
            should_reverse = True
        elif hit_wall:
            should_reverse = True
